...
public class WaterFallListView extends MultiColumnListView implements OnScrollListener {
	...
	// -- header view
	private WaterFallViewHeader mHeaderView;
	...
	private void initWithContext(Context context) {
		...
		// WaterFallListView need the scroll event, and it will dispatch the event to
		// user's listener (as a proxy).
		super.setOnScrollListener(this);

		// init header view
		mHeaderView = new WaterFallViewHeader(context);
	...
	public void setAdapter(ListAdapter adapter) {
		// make sure WaterFallListViewFooter is the last footer view, and only add once.
	...
	private void updateHeaderHeight(float delta) {
		...
		if (mEnablePullRefresh && !mPullRefreshing) { // 未处于刷新状态，更新箭头
			if (mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
				mHeaderView.setState(WaterFallViewHeader.STATE_READY);
			} else {
				mHeaderView.setState(WaterFallViewHeader.STATE_NORMAL);
			}
		}
		...
		default:
			...
			if (getFirstVisiblePosition() == 0) {
				...
				if (mEnablePullRefresh && mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
					...
					mHeaderView.setState(WaterFallViewHeader.STATE_REFRESHING);
					if (mListViewListener != null) {
